...
pub struct DefPin {
    ...
    pub net: String,  // aux.net
    pub direction: String, // aux.net
}
...
impl PinValidator {
    fn build(bookshelf: &Bookshelf) -> Self {
        let by_route: BTreeSet<String> =  bookshelf.route.ni_terminal_to_layer.keys().cloned().collect();
        let by_pl: BTreeSet<String> = bookshelf.pls.iter().filter(|x| matches!(x.r#type, crate::pl::Type::FixedNotInImage)).map(|x| {
            x.name.clone()
        }).collect();
    ...
    fn is_valid(&self) {
        ...
        for (pin_name, port_cnt) in self.pin_in_net.iter() {
            if *port_cnt >= 2 {
                panic!("pin {} has more than one port. I think this is not valid", pin_name);
            }
        }
...
impl DefPins {
    pub fn write(&self) -> String {
        ...
        for def_pin in self.0.values() {
            res += &format!("\
                ...
                def_pin.net,
                def_pin.direction,
                def_pin.layer, (def_pin.shape.x * 1000.0) as i64, (def_pin.shape.y * 1000.0) as i64,
                (def_pin.place.x * 1000.0) as i64, (def_pin.place.y * 1000.0) as i64
            );
        }
        res += &format!("END PINS");
        res
    ...
    pub fn build(bookshelf: &Bookshelf) -> Self {
        ...
        let mut cnt_pl = 0;
        for pl in bookshelf.pls.iter().filter(|x| matches!(x.r#type, crate::pl::Type::FixedNotInImage)) {
            cnt_pl += 1;
            let pin = res.get_mut(&pl.name).unwrap();
            pin.place = pl.place.clone();
            pin.orientation = pl.orientation.clone();
        }

        let mut cnt_node = 0;
        for node in bookshelf.nodes.iter().filter(|x| matches!(x.moveable, crate::nodes::Movable::FixedButOverlapAllowed)) {
            cnt_node += 1;
        ...
        for net in bookshelf.nets.iter() {
            for net_pin in net.pin.iter() {
                if let Some(pin) = res.get_mut(&net_pin.instance_name) {
                    ...
                    cnt_net += 1;
                    pin.net = net.name.clone();
                    pin.direction = match net_pin.pin_name.as_bytes() {
                        b"I" => format!("INPUT"),
                        b"O" => format!("OUTPUT"),
                        _ => panic!("Terminal_NI considered as PIN which usually have only one direction, INPUT or OUTPUT"),
                    };
                }
            }
        }
